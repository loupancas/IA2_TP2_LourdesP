...
public class EnemigoVolador : EnemigoBase, IFreezed, IGridEntity
{
    ...
    public Vector3 Position  // Implementaci�n de IGridEntity
    {
    ...
    public override void Morir()
    {
        if (GameManager.instance.IsInGridState())
        {
            GameManager.instance.arenaGrid.enemigosEnLaArena.Remove(this);
        }
        else
        {
            GameManager.instance.arenaFSM.enemigosEnLaArena.Remove(this);
        }
    ...
    private void Update()
    {
        ...
        if (_position != transform.position)  // A�adido
        {
    ...
    void Reset()
    {
        ...
        if (!gameObject.activeInHierarchy)
        {
            if (GameManager.instance.IsInGridState())
            {
                GameManager.instance.arenaGrid.enemigosEnLaArena.Add(this);
            }
            else
            {
                GameManager.instance.arenaFSM.enemigosEnLaArena.Add(this);
            }
        }
    }
    ...
    {
        foreach (EnemigoBase a in GameManager.instance.arenaFSM.enemigosEnLaArena)
        {
            if (a == this)
                continue;
    ...
    //IA2-TP2 Aggregate identifica el punto el punto m�s d�bil y lo activa
    ...
    //IA2-TP2 Aggregate calcula el da�o total recibido por el enemigo y lo muestra en pantalla
