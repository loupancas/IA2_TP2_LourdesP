...
public class FirstPersonPlayer : Entity
{
    ...
    public void CambioDeArma()
    {
        ...
        int currentWeaponIndex = _weaponStash.IndexOf(equippedWeapon);
        List<int> availableIndices = new List<int>();

        for (int i = 0; i < _weaponStash.Count; i++)
        {
        ...
        int numeroRandom = availableIndices[UnityEngine.Random.Range(0, availableIndices.Count)];
    ...
    //     int currentWeaponIndex = _weaponStash.IndexOf(equippedWeapon);
    //     int nextWeaponIndex = (currentWeaponIndex + 1) % _weaponStash.Count;

    //     equippedWeapon = _weaponStash[nextWeaponIndex];
    //     _buffs.ArmaUpdate(equippedWeapon);
    //     _inputs.UpdateWeapon(equippedWeapon);
    ...
    public void AgregarBuff()
    {
        ...
        switch (Numero)
        {
            case 0:
                ...
                break;
            case 1:
                _buffs.Add("Da�o Aumentado", (Stats original) => { original.cooldownFreeze -= 1; return original; }, 5);
                print("Da�o Aumentado");
                break;
            case 2:
